...
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SANPP_RF_VALORIZACAO_LOTE_THIAGO]'))
BEGIN
	DROP PROC SANPP_RF_VALORIZACAO_LOTE_THIAGO
	--SELECT 1
END
GO

CREATE PROCEDURE DBO.SANPP_RF_VALORIZACAO_LOTE_THIAGO
 (  
	...
	BEGIN 
		...
		IF @USU_CODIGO <> '' and @GERAR_LOG_BATCH = 'S'  
		  BEGIN  
		   ...
		   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SANPP_RF_VALORIZACAO / SANPP_RF_VALZ_ESTQ_GERENCIAL - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)) )  
		  END  
...
if @@spid = 117  select 'rf2500', 145
if @@spid = 117  select @DT_ICAL DT_ICAL
...
if @@spid = 117  select 'rf2500', 178
if @@spid = 117  select @DT_VAL_INICIO
if @@spid = 117  select 'rf2500', 180
if @@spid = 117  select @DT_FNAL
		   ...
		   BEGIN  
			...
			-- RF0888.PRC  
				SELECT @MENSAGEM_ERRO = ''  
if @@spid = 117  select 'rf2500', 187
			EXEC @RETORNO = SANPP_RF_VALORIZACAO @CARAC, @DT_VAL_INICIO, @DT_FNAL, @RETORNA_MSG, @MENSAGEM_ERRO OUTPUT  
if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
 ...
 --FLC INI              
  ...
  --IF ISNULL(RTRIM(@TIPO_PROCESSO), '') = 'Valorizacao Nao Padronizadas'  AND ISNULL(RTRIM(@ATV_CODIGO), '') = ''               
  --BEGIN               
  --    SELECT @WHERE = @WHERE + '                
  --     AND A.ATV_CODIGO NOT IN (''CRI'',''CRA'',''DEBENTURES INCT'')'              
  --END              
  --ELSE              
  --BEGIN                
...
--FLC FIM      
 ...
 -- SELECT  
 --  @WHERE = @WHERE + '  
 --  AND A.RF_CARACTERISTICA NOT IN (SELECT TXT_DESC_EVE FROM SANT1984_RF_CONTROLE_VALORIZACAO (NOLOCK) WHERE DT_B = ''' + CONVERT(CHAR(08), @DT_PROCESSO, 112) + ''' AND SGL_T_RG = ''CARAC'')'  
...
if @@spid = 117  select 'rf2500', 653
if @@spid = 117  SELECT @STR
...
--  SELECT 'BICUDO 6'  
 ...
 -- SELECT '#OPRC_NAO_PADRONIZADA_SEM_FLUXO 1'  
 -- SELECT * FROM #OPRC_NAO_PADRONIZADA_SEM_FLUXO  
 -- SELECT '#OPERACOES1'  
 -- SELECT * FROM #OPERACOES  
...
--  SELECT * FROM #OPERACOES_GERAL  
 ...
 --SELECT '#OPERACOES_GERAL - depois do delete'  
 --SELECT * FROM #OPERACOES_GERAL  
...
--  SELECT '#OPERACOES_GERAL - depois do segundo delete'  
--  SELECT * FROM #OPERACOES_GERAL  
...
--   SELECT '#OPERACOES_GERAL - antes de validar estoque_ori para delecao'  
--   SELECT ATV_FCVALORIZ, ESTOQUE_ORI, IDX_CODIGO, IDX_PRE, ATV_PUBLICO, IC_FIDC FROM #OPERACOES_GERAL  
  --drop table ##operacao_Geral_copia  
...
--SELECT '#OPRC_NAO_PADRONIZADA_SEM_FLUXO 2'  
      
--    SELECT '#OPERACOES_GERAL - depois de validar estoque_ori'  
--    SELECT ATV_FCVALORIZ, ESTOQUE_ORI, IDX_CODIGO, IDX_PRE, ATV_PUBLICO, IC_FIDC FROM #OPERACOES_GERAL  
...
--   
-- SELECT '#OPRC_NAO_PADRONIZADA_SEM_FLUXO 3'  
-- SELECT * FROM #OPRC_NAO_PADRONIZADA_SEM_FLUXO  
        -- gravar log de todas as operações que foram desviadas  
 -- #OPRC_NAO_PADRONIZADA_SEM_FLUXO  
  
--  SELECT 'BICUDO 4 - #OPRC_NAO_PADRONIZADA_SEM_FLUXO'  
-- -- SELECT * FROM #OPRC_NAO_PADRONIZADA_SEM_FLUXO  
-- SELECT * FROM #OPRC_NAO_LOTE  
        DELETE ONL  
        FROM   #OPRC_NAO_LOTE ONL  
...
-- SELECT  * FROM #OPERACOES_GERAL  
--   
-- SELECT '#OPRC_NAO_PADRONIZADA_SEM_FLUXO'  
-- SELECT  * FROM #OPRC_NAO_PADRONIZADA_SEM_FLUXO  
--   
-- SELECT '#OPRC_NAO_LOTE'  
-- SELECT * FROM #OPRC_NAO_LOTE  
...
-- SELECT * FROM #OPRC_LOTE  
 ...
 BEGIN  
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SANPP_RF_VALORIZACAO_PRIVADOS_PRE - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)))  
  END  
  -- CALCULOS PRIVADOS PRE  


 if @@spid = 117  select * from #OPERACOES
 if @@spid = 117  select 'rf2500', 1714
  EXEC SANPP_RF_VALORIZACAO_PRIVADOS_PRE @DT_ICAL, @DT_FNAL, @GERAR_LOG_BATCH, @DT_PROCESSO, @CONSIDERAR_RETROATIVAS  
 if @@spid = 117  select top 10 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  
  -- LOG   
  IF @USU_CODIGO <> '' and @GERAR_LOG_BATCH = 'S'  
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Fim: SANPP_RF_VALORIZACAO_PRIVADOS_PRE  - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)))  
  END  
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SANPP_RF_VALORIZACAO_PRIVADOS_CDI - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)))  
  END  
  -- CALCULOS PRIVADOS CDI E RESERVA  
--SELECT 'BICUDO 1 - CDI LOTE -- #OPERACOES_GERAL'  
--SELECT * FROM #OPERACOES_GERAL  
-- if @@spid = 117  select 'rf2500', 1741
...
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SANPP_RF_VALORIZACAO_CALCULOS_SALDOS - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)))  
  END  
  
  -- CALCULO DAS OUTRAS COLUNAS DA RF_SALDOS E INSERCAO NA BASE  
-- if @@spid = 117  select 'rf2500', 1770
...
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SANPP_RF_VALORIZACAO_LOTE_ESTQ_GER - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)))  
  END  
  
  -- ESTOQUE GERENCIAL  
-- if @@spid = 117  select 'rf2500', 1795
...
--  SELECT 'BICUDO 3 - #TB_OPERACOES_NAO_LOTE'  
--  SELECT COUNT(*) FROM #TB_OPERACOES_NAO_LOTE  
--  SELECT 'BICUDO 3 - #OPRC_LOTE'  
--  SELECT * INTO ##TB_OPERACOES_NAO_LOTE FROM #TB_OPERACOES_NAO_LOTE  
...
--    SELECT  
--     @CARAC = MIN(RF_CARACTERISTICA)  
--    FROM  
--     #TB_OPERACOES_NAO_LOTE  
--    WHERE  
...
--      RF_CARACTERISTICA = @CARAC  
--   
--     IF @TEM_MOVTO_RETROATIVO = 'S'  
--     BEGIN  
--      -- VALORIZA A PARTIR DO ULTIMO DIA UTIL, QUE E EXATAMENTE A DATA DO MOVIMENTO RETROATIVO  
--      SELECT @DT_VAL_INICIO = @DT_RESERVA_ANTERIOR  
--     END  
--    END  
...
--      RF_CARACTERISTICA = @CARAC  
--   
--     IF @DT_ULT_COTA_LIBERADA >= @DT_VAL_INICIO  
--     BEGIN  
--      -- VALORIZA SOMENTE A PARTIR DA DATA DA ULTIMA COTA LIBERADA  
--      SELECT @DT_VAL_INICIO = DATEADD(DAY, 1, @DT_ULT_COTA_LIBERADA)  
--     END  
--    END  
...
--SELECT 'BICUDO 2 - SANPP_RF_VALORIZACAO', @CARAC CARAC  
  
    -- RF0888.PRC  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 1984
    EXEC @RETORNO = SANPP_RF_VALORIZACAO @CARAC, @DT_VAL_INICIO, @DT_FNAL, @RETORNA_MSG, @MENSAGEM_ERRO OUTPUT  
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
    ...
    /***************************************************/  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 2003
    EXEC SANPP_RF_VALZ_ESTQ_GERENCIAL @CARAC, @DT_VAL_INICIO, @DT_FNAL, @RETORNA_MSG, @MENSAGEM_ERRO OUTPUT  
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SIAN_RF_VAL_FIS_003 / SANPP_RF_VALZ_ESTQ_GERENCIAL - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)) )  
  END  
...
--    END  
   ...
   BEGIN  
    ...
    -- GBVLFI03.PRC  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 2170
...
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
    ...
    /***************************************************/  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 2189
    EXEC SANPP_RF_VALZ_ESTQ_GERENCIAL @CARAC, @DT_VAL_INICIO, @DT_FNAL, @RETORNA_MSG, @MENSAGEM_ERRO OUTPUT  
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Fim: SIAN_RF_VAL_FIS_003 / SANPP_RF_VALZ_ESTQ_GERENCIAL - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)) )  
  END  
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Início: SIAN_RF_VAL_FIS_005 / SANPP_RF_VALZ_ESTQ_GERENCIAL - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)) )  
  END  
...
--    END  
   ...
   BEGIN  
    ...
    -- GBVLFI05.PRC  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 2348
...
-- if @@spid = 117  select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
    ...
    /***************************************************/  
        SELECT @MENSAGEM_ERRO = ''  
-- if @@spid = 117  select 'rf2500', 2368
    EXEC SANPP_RF_VALZ_ESTQ_GERENCIAL @CARAC, @DT_VAL_INICIO, @DT_FNAL, @RETORNA_MSG, @MENSAGEM_ERRO OUTPUT  
-- if @@spid = 117  
select top 3 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos (nolock) where rf_caracteristica = '00000000000023021639' order by rfs_data desc
  ...
  BEGIN  
   INSERT INTO HISTORICOS_VALORIZACAO (USU_DATA_HORA,USU_CODIGO,STRU_MODULO,SIS_CODIGO,DESC_OPER)  
   VALUES ( GETDATE(),@USU_CODIGO,'RFVEXE','RFV','Fim: SIAN_RF_VAL_FIS_005 / SANPP_RF_VALZ_ESTQ_GERENCIAL - Conexão:' + convert(char(50),isnull(@ID_CONEXAO,0)) )  
  END  
...
-- select top 15 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos where rf_caracteristica = '00000000000023021639' order by rfs_data desc
...
exec sanpp_rf_valorizacao_harry '00000000000013861200', '2019-08-19', '2019-08-19'

EXEC SANPP_RF_VALORIZACAO_HARRY '00000000000017475880','2018-11-19','2018-11-19'

select top 15 rfs_pu_uteis, rfs_pu_corridos, * from rf_saldos where rf_caracteristica = '00000000000013861200' order by rfs_data desc


EXEC SANPP_RF_CALC_PU_DIA 00000000000017475880,  -- OPERACAO A SER CALCULADA.  
        2018-11-19 00:00:00,   -- DATA DE CALCULO DO PU  
        1504543,   -- IDENTIFICADOR DO TITULO  
        1,   -- IDENTIFICADOR DO PERIODO DE REPACTUACAO OU ZERO PARA PADRONIZADOS  
        N,  -- INDICADOR DE OPERACAO NAO PADRONIZADA  
        2014-01-15 00:00:00,   -- EMISSAO DO TITULO  
        2019-05-15 00:00:00,   -- VENCIMENTO DO TITULO  
        6,   -- CUPOM DO TITULO  
        022,   -- FORMA DE CALCULO DE JUROS  
        013,   -- FORMA DE CALCULO DO CUPOM  
        IPCA           ,   -- INDEXADOR DE CORRECAO  
        2018-11-19 00:00:00,   -- DATA DE AQUISICAO DO TITULO  
        2014-01-15 00:00:00,   -- DATA DE INICIAL DA REPACTUACAO  
        2019-05-15 00:00:00,   -- DATA DA PROXIMA REPACTUACAO  
        2000-07-15 00:00:00,   -- DATA BASE DE CALCULO DO TITULO  
        100,   -- PERCENTUAL DO INDEXADOR  
        SAFRABM        ,   -- POSICAO PARA FERIADOS  
        N,   -- FLAG PARA FUNDO OU NAO  
        S,   -- C ETIP PARA PRIVADOS OU  S ELIC PARA PUBLICOS  
        6,  -- PRAZO DE PAGAMENTO DE JUROS PARA NTNs E NBC-Es  
        1000,   -- NOMINAL DO TITULO PARA NTNs E NBC-Es  
        C ,   -- CURVA DE CONTABILIZACAO  
        005,   -- FORMA DE CALCULO DA CURVA MERCADO  
                       ,   -- SIGLA DA MOEDA APC  
        S,   -- INDICADOR DE CENARIO DO VENCIMENTO  
        CDI            ,   -- INDEXADOR GERENCIAL PARA PU MANUAL  
        CDI            ,   -- INDEXADOR DE CUSTO PARA MTM PRE  
        S,   -- INDICADOR DE CALCULO DO CENARIO EM DATA DE AQUISICAO  
        4.38637,    -- TIR UTEIS  
        4.27244,    -- TIR CORRIDOS  
